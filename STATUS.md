...
**NRM2 cost model — populates Brief A's £ placeholders.** Architect: Claude Chat; authorised by Chris. Brief: [`active/90_nrm2_cost_model.md`](docs/briefs/active/90_nrm2_cost_model.md); design note: [`design-notes/brief_B_nrm2_cost_model.md`](docs/design-notes/brief_B_nrm2_cost_model.md); audit: [`audit/90_nrm2_cost_model.md`](docs/audit/90_nrm2_cost_model.md). **Part 1 landed (docs + audit stub).** **Part 2 BLOCKED:** the **Applemore Leisure Centre Feasibility Cost Plan spreadsheet is not in the repo** (no `docs/reference/`; only `Bridgewater_Bottom_Up_Energy_Model.xlsx` exists). The entire rate library + NRM2 elemental templates + on-cost defaults derive from it. Cannot proceed without fabricating rates (CLAUDE.md rule 2 forbids). **Need Chris to drop the Applemore spreadsheet at `docs/reference/applemore_cost_plan.xlsm`.** Last in the A→C→B sequence; closes into the combined PR.
