...
- **P1 — lifecycle £/tonne:** £/tonne uses LIFECYCLE capex = initial + 70%·initial per measure-life expiry before 2050 (`floor(25/life)` replacements; controls 10→2, plant 15→1, PV 25→1, fabric ≥26→0). Replacement = 70% (excludes one-off strip-outs/supply/builder's-work/design ≈30%). Only £/tonne uses it — Capex column + payback keep initial capex; carbon stays integrated to 2050. `measure_life_years` seeded onto all 22 from `scripts/report/interventions.py` bands (`_brief101_seed.py`). **Verified:** DHW ASHP (plant 15) £/t **155→264** (1 replacement ×1.70); fabric (30y) unchanged; controls (10y) ×2.4.
- **P2 — assumption notes:** new `assumption_notes` field (ENERGY BASIS / COST BASIS), auto-drafted for all 22 from the design-note `assumption`/`basis` + cost `tier`/`on_cost_pct`/`confidence`. Rules honoured: "assumption:" flagged explicitly; the four L-flags (1.2 WWHR, 1.3 exhaust-air, 1.5 interlink, 2.1 MVHR) say allowance-only pending survey; PV (7.1) states the CRREM gross-demand rule (carbon saving, EUI unchanged); no invented sources. Editable via a plain textarea in the isolated view (`PerInterventionView`); exported as a 5th **Assumptions** sheet (Label · Measure life · ENERGY BASIS · COST BASIS).
- **Verified:** build clean; three notes (WWHR/DHW-ASHP/interlink) read correctly against sources; browser check — textarea renders, no console errors, **UI ↔ export consistent** (1.1 £95, 1.4 £264 in both, lifecycle applied). DB fields intact on all 22.
