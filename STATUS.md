...
### 🗒️ Logged for separate consideration — NZA-Sim instant-engine ↔ v40 split (NOT an EP-derive defect)

The four fields the config-drift audit flagged (`lighting_control`, `ashp_cop_dhw`, DHW service-level setpoints, `dhw_preheat`) are **not** EnergyPlus-derive drift — 98-pre-b's preserve-from-simple correctly mirrors what NZA-Sim's instant engine reads. The residual is an **upstream inconsistency inside NZA-Sim itself**: its instant engine (demand/gain + DHW-thermal path → the 132.6/126.0 anchors + displayed Results) reads these from the **flat/simple config** (`instantCalc.js:6050` ← `ProjectContext.jsx:789`; `instantCalc.js:6138-6143`), while a **separate** systems-electricity path reads **v40** (`systemsEngine.js` `control_factor`, `effectiveSystemScalar`). So NZA-Sim holds two answers for the same fields.

- **Not fixed here** (would require `instantCalc.js` changes → deliberate physics change that moves the anchors; out of scope for a config-derive brief).
- **Depends on a product decision, not a bug fix:** the DHW case is the sharp one — instant engine = **gas-only**; v40 = **52% gas / 48% ASHP at COP 3**. Which is "the building"? The report's DHW baseline rides on it. Same shape for `lighting_control` (instant `occupancy_sensing` 0.80 vs v40 `constant` 1.0).
- **Owner:** Chris — this is a "what is the building" call. When decided, a follow-up brief either (a) makes the instant engine read v40 for these (closing NZA-Sim's own split, then the EP derive follows automatically), or (b) confirms the simple values are canonical and updates v40 to match. Either way the EP derive stays correct-by-construction (it mirrors the instant engine).
- Evidence: [`audit/98prec_escalation.md`](docs/audit/98prec_escalation.md).
