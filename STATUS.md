...
Investigated the audit's four (c) findings before extending the derive. **The audit's premise was inverted:** NZA-Sim's *instant engine* (behind the 132.6/126.0 anchors + displayed Results) reads `lighting_control` and DHW from the **flat/simple config**, NOT v40 (`instantCalc.js:6050` ← `ProjectContext.jsx:789`; `instantCalc.js:6138-6143`). The EP assembler is explicitly kept in sync with instantCalc (`instantCalc.js:86`), so 98-pre-b's *preserved* values **already match** the instant engine (lighting 0.80 = 0.80). Deriving from v40 would have opened a ~20% EP-vs-anchor lighting gap — moving the drift, not closing it. **STOP-and-write** per the brief's escalate clause; derive/assembler/instantCalc untouched. Resolution: the dangerous drift (system *type*) is fully closed by 98-pre-b; the residual (c) items are an **upstream NZA-Sim instant-engine split** (simple fields vs v40), not an EP-derive defect — an `instantCalc.js` change, out of scope. Audit doc corrected. **Open for Chris (not Code's):** is Bridgewater DHW gas-only (simple) or ASHP-48%-COP-3 (v40)? — a "what is the building" decision the report depends on. Deliverable: [`audit/98prec_escalation.md`](docs/audit/98prec_escalation.md) + CORRECTION in [`audit/config_drift_rootcause.md`](docs/audit/config_drift_rootcause.md).

### 🔎 Resolved by definitive read-only trace (2026-07-09) — DHW report-safe; lighting is a real EP under-count

**Correction:** my earlier 98-pre-c escalation ([`audit/98prec_escalation.md`](docs/audit/98prec_escalation.md)) claimed the *displayed* instant engine reads these fields from the simple config. That was traced through the wrong function — **`calculateInstantDegreeDay`** (`instantCalc.js:5971+`, the lines 6050/6138 I cited), a **legacy degree-day helper that does NOT produce the displayed numbers**. The displayed consumption + the 132.6/126.0 anchors come from **`_calculateState3`** (`instantCalc.js:4941`), which reads **v40**. Re-traced both fields definitively against the **live Bridgewater** project:

- **DHW — RESOLVED, report-safe.** `_calculateState3` → `computeSystemsDelivered(v40)` → `dhw = dhw_v40_block ?? dhw_v25` (`instantCalc.js:5176`) reads **v40**. Live displayed `consumption.dhw` = **electricity 42.2 MWh + gas 157.4 MWh** (heat-pump 60% of demand share) — the ASHP **is** present, not gas-only. The EP path (98-pre-b) also adds the ASHP preheat, so **EP and the displayed engine agree on the DHW carrier.** Minor residual only: EP models the ASHP at the default **COP 2.8** vs v40's **COP 3** (~7% on the ASHP-electricity portion — the old "C2"), a small refinement, not gas-vs-ASHP. **No "what is the building" ambiguity in what's displayed — v40 (hybrid) is what NZA-Sim shows.**

- **Lighting — a REAL EP-side under-count; the audit's original C1 was correct.** `_calculateState3` scales lighting by **`effectiveSystemScalar(building.systems_config_v40.lighting)`** (`instantCalc.js:2418`, uses v40 `share_pct × control_factor`). Live v40 lighting = `control_factor 1.0` (constant) → displayed lighting **44.46 MWh** (full LPD, no reduction). But the **EP path** (98-pre-b derive) *preserves* the stale simple `lighting_control='occupancy_sensing'` → factor **0.80** → EP lighting ≈ **35.6 MWh, ~20% below what NZA-Sim displays.** So my escalation's "preserve-from-simple already matches the instant engine" was **wrong for lighting**; the audit's **C1 (derive `lighting_control` from v40) stands.** The fix is EP-derive-only — map v40 `control_mechanism` → `lighting_control` so the EP factor becomes 1.0 — **no `instantCalc.js` change, does not move the anchors.**

**Net:** DHW needs no action (optional C2 COP refinement aside). **Lighting warrants a small, bounded EP-derive fix** (C1) so `/api/simulate` lighting matches the displayed 44.46 MWh. Owner: Chris — decide whether to land the C1 lighting derive now (EP-only, ~one mapping in `derive_systems_for_sim`) or hold it for Brief 98 P0.

⚠️ The on-main [`audit/config_drift_rootcause.md`](docs/audit/config_drift_rootcause.md) CORRECTION section and [`audit/98prec_escalation.md`](docs/audit/98prec_escalation.md) still carry the superseded "displayed reads simple" claim — flagged for a doc-only follow-up correction. Trace evidence: this entry + the read-only run on live project `12cf7cc4`.
