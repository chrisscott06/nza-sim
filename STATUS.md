...
**Part 5 — Strategy page DONE (core), browser-verified.** Built `StrategyView.jsx` (replaces VisualiserHost on the Strategy page): strategy headline (final EUI + energy/carbon saved + B/C placeholders), waterfall, final-state heat balance (reuse PhysicsView, Rows/Stacked/Sankey), CRREM placeholder. Narrowed list to 440px. Also authored **6 real Bridgewater interventions** from Chris's list (DHW ASHP-only SCOP5 −41.9, plug-load −14.0, LED −3.4, setpoints −3.3, brise soleil −1.0, bedroom-extract→MVHR **+3.8 increase** — a genuine "cooling-leaning building" insight). Verified at 1440×900: final EUI **76.2** from 138.3 (−62.1 kWh/m² / −261.6 MWh/yr); waterfall shows order-dependent marginals; heat balance + CRREM render; no console errors; build clean. Pending refinements: heat-balance compare button, distinct Library/Strategy subset, manual drag-reorder test. Parts 6–7 pending.
