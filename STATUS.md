...
## 🔨 Brief 97 — Interventions Studio (module redesign + RICS cost editor as pop-out) — IN PROGRESS 2026-07-08 (overnight, unattended) on `chris/cost-plan-editor` *(cut from `main` `d7d2c37` post-95/96 merge; do NOT merge unattended — Chris walkthrough + independent review gate close)*

**Supersedes Brief 91 + the 91b stub/editor drafts.** Turns the Interventions module from a grey-scroll into a designed, tabbed workspace (Impact/Carbon/Demand/Cost with semantic colour), restyles the Strategy view + validate panel, and replaces the transitional headline cost card with the full RICS/NRM2 cost editor as a **pop-out** window (overrides 91's inline + UX-freeze choices). Carries forward the 91b P2 work — CostPlanEditor component, migrate-on-read, transitional-block removal — rehomed into the pop-out. **Zero physics** — `_brief93_anchor.mjs --fixture` EUI **132.6** captured at start; byte-identical at close.

- **P1** — B97 landed (`docs/briefs/active/97_interventions_studio.md`); 91b drafts `git rm`'d from active/ (91 kept as content source); session reconcile. EP-flag-rename rider already applied (91b P1, commit `c94ff24`): in `scripts/report/build_tables.py`, "EP-validated ✓" softens to "EP-checked — see Table 3" where EP EUI \|Δ%\| > 25 (Chris's chosen metric — the brief's "(currently 3.3)" was stale; trips **2.2, 3.3, 3.4, 3.5**).
- **From 91b (on-branch, reused):** P1 rider (above) `c94ff24`; P2 `0589b43` — `CostPlanEditor.jsx` (hierarchical group/line table, B94 drag pattern, unit-adaptive labels, on-cost cells, NRM2 sequence), `HeadlineCostEditor` deleted, both transitional `costModel.js` blocks stripped, migrate-on-read wired in ProjectContext. NRM2 math browser-verified (2×£18,000 → Total £56,160).
- **P2–P9** — in progress (semantic colour · tabbed isolated view · cost model + migration · pop-out editor · keyboard · HIEX templates · Strategy restyle · close).
