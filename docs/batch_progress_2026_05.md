...
**Current state:** `paused_for_walkthrough` (Brief 28a Part 3, 3 of 6 slices done — awaiting Chris's walkthrough on 3a/3b/3c before 3d/3e/3f)
...
| 3. Canvas tab restructure (sliced 3a-3f) | **3a/3b/3c done; halted for walkthrough; 3d-3f pending** | 2026-05-14 | — | `7782556` (3a), `8b33206` (3b), `496cda3` (3c) | **3a** added Summary tab as new default; **3b** folded Delta content into Summary + added gains-vs-demand stacked bar with kWh / kWh-per-m^2 unit toggle + removed standalone Delta tab; **3c** consolidated Free-running + Hourly + Annual breakdown into single Load shape tab with sub-view toggle (interim — Parts 4-5 replace with unified Pavlo time-series). Tab strip post-3c: Schedule \| Summary \| Heat balance \| Load shape \| 3D Model (5 tabs, down from 7). Walkthrough targets in STATUS.md. **3d-3f queued:** remove 3D Model from Internal Gains, apply tab pattern to Building, update ui_principles.md. |
