...
**Current state:** `paused_for_walkthrough` (2026-05-14, after Brief 28a Parts 1-2)
**Last update:** 2026-05-14
**Pause rationale:** 11 commits shipped this session covering Brief 27 cleanup (closed), Brief 28 prereq (closed, via Option C+), and Brief 28a Parts 1-2 (Static/Dynamic rename + corrected disclosure + kWh/m²·yr readouts). All UI changes are paper until Chris loads `/gains` / `/results` / `/information` in the browser. Brief 28a Part 3 (canvas tab restructure) is a multi-module refactor that deserves a clean context budget — postponed to a fresh conversation. **Standing order for the new conversation:** proceed per the orchestration doc starting at `docs/briefs/active/28a_visible_polish.md` Part 3 (slicing plan already in the brief). Pre-flight checks expected to pass since everything shipped this session has full regressions green + build clean. Backend uvicorn process needs a restart to pick up the `simulation_mode` column changes in `/simulations` and `/simulate` endpoints.
