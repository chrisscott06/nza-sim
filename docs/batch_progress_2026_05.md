...
**Current state:** `paused_for_walkthrough` (Brief 28a Part 5 done — Conditions tab rewritten with Pablo composition + lens selector + ZoomNav/MonthJumpButtons wired to real Bridgewater data; awaiting walkthrough on the live `/gains` Conditions tab before 3e.)
...
| 4. Pablo component port | **done + walkthrough refinement; halted for re-walkthrough** | 2026-05-14 | 2026-05-14 | `c54ee6f` (port) → `042dc84` (test harness) → (this commit: composition fix + principles update) | **Initial port** (`c54ee6f`): read actual Pablo sources from `pablo-2/`. NZA-Sim already had clean equivalents of ChartContainer + DataCard; real new ports were ZoomNav + MonthJumpButtons + 4 new tokens. ChartContainer + DataCard moved `ui/` → `chart/` for canonical-location alignment; 9 downstream imports updated. **Test harness** (`042dc84`): `/chart-test` route. **Walkthrough feedback 2026-05-14:** chart fills viewport height (wrong), DataCards stacked above/below at full width (should be 2-col with chart left + stats stacked right), density too low (looks like marketing page). **Refinement (this commit):** (i) `ui_principles.md` updated with Principle 6 (density baseline) + new "chart paired with stat panel" pattern + chart-height rules; (ii) ZoomNav density tightened (text-caption → text-xxs, smaller padding/icons); (iii) `/chart-test` rewritten with canonical composition (ZoomNav above + chart-left + DataCards-stacked-right + MonthJumpButtons below + synthetic daily trace as data + comfort-band reference lines). These layout rules now land in `ui_principles.md` BEFORE Part 5 / 3e so they inherit cleanly. |
| 5. Conditions + engine toggle | **done — halted for live-data walkthrough** | 2026-05-14 | 2026-05-14 | (this commit) | LoadShapeView rewritten: interim 3-sub-view toggle (Temperature / Hourly / Annual breakdown) replaced with the Pablo composition pattern. Single bounded card: lens selector (Temperature \| Gain profile) + EngineBadge at top right; ZoomNav full width above chart; chart left (300px height, Recharts LineChart for Temperature, AreaChart stacked for Gains); DataCards stacked right in 180px column (lens-specific stats — Peak/Trough/Mean/In-band for Temperature; Peak/Mean/People-share/Lighting-share/Equipment-share for Gains); MonthJumpButtons below chart full width. Annual-breakdown lens DROPPED (not a time-varying signal; per-gain attribution already in Summary tab — flagged for walkthrough revisit). Engine toggle wiring deferred — EngineBadge ships as label only; the Live/Simulation segmented control + State 2 EP results plumbing remains the Brief 27 9/10 holdback to close in this Part once SQL parser per-profile breakdown (Brief 28a Part 5 SQL chunk per original brief) is added in a follow-up commit, OR deferred to Brief 28a Part 7 close-out. Lens decision rationale (Chris's UX question): (a) toggle, not (b) stacked, not (c) overlay — incompatible y-axes between temp/gains rule out overlay. Build clean (17.20s). State 1 + State 2 Live regressions byte-identical. |
