...
| 3 | 2026-05-14 | 28a (paused) | (walkthrough Finding 2) | SH3-adjacent (slow Static engine perceived) | **Resolved 2026-05-14 via fix-path (b).** Walkthrough revealed ~1 minute delay perceived as Static engine slowness. Profiled engine: ~25ms cold. Real cause: auto-simulate firing full Dynamic EP run on saves. Chris chose option (b): gate auto-simulate on `saveSource === 'user'`. Shipped: `ProjectContext.jsx` adds `saveSource` state, `_scheduleSave(..., source)` accepts source param (defaults to `'system'` for fail-safe), 5 existing user-edit call sites explicitly tag `'user'`. `SimulationContext.jsx` reads `saveSource` and gates auto-simulate accordingly. Build clean, State 2 Live regression byte-identical. Browser verification (no surprise EP run on project load; user edit still triggers Dynamic after 2s) pending Chris's walkthrough — when confirmed, batch state flips from `paused_for_walkthrough` to `running`. |
