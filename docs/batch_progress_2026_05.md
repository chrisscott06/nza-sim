...
**Current state:** `paused_for_walkthrough` (halt 3 — Finding 2 fix-path needs Chris's call)
**Last update:** 2026-05-14
**Walkthrough findings:**

- **Finding 1 (Heat balance bug) FIXED.** Brief 27 cleanup Part 1's prop rename was correct but incomplete. Added Part 3 (corrected close) — unwrapped `state2.heat_balance` in the wrapper + moved internal gains under `gains.internal.*` in `_calculateState2`. New diagnostic script `scripts/verify_state2_heat_balance_shape.mjs` confirms 15/15 shape checks. State 1 + State 2 Live regressions byte-identical post-fix. Brief 27 cleanup overall confidence revised: 10/10 → **9/10** (the gap is the missed shape verification, captured as a learning + a regression-test candidate for Brief 28a Part 7).
- **Finding 2 (slow State 1 → State 2 transition) ROOT CAUSE IDENTIFIED.** Profiled the Static engine via new `scripts/profile_static_engine.mjs`: state1 cold 7.8 ms, state2 cold 23.5 ms, warm 1-17 ms. Engine is fine. The "~1 minute" delay is auto-simulate firing a full Dynamic EP run in the background (`SimulationContext.jsx:59` defaults `autoSimulate=true`; `:92-115` triggers `runSimulation()` 2 s after every save, including project-load normalisations/migrations). Full mode EP runs take ~35-45s. **Fix-path needs Chris's call** — three options in halt report.

Brief 28a Part 3 (canvas tab restructure) remains blocked until Finding 2 resolves.
...
| 3 | 2026-05-14 | 27 cleanup (re-opened) | 1 (Heat balance prop fix — incomplete) | (verification gap, not in HH/SH list) | **Resolved 2026-05-14 via Part 3 (corrected close).** Chris's walkthrough caught that the empty state still rendered despite Part 1's 10/10 close. Root cause: prop name was right but shape needed unwrapping AND engine output needed restructuring. Fixed both. Brief 27 cleanup confidence revised 10/10 → 9/10. See `docs/briefs/archive/27_cleanup_COMPLETED.md` Part 3 section for the full account. |
| 4 | 2026-05-14 | 28a (paused) | (n/a — pre-Part-3 finding) | SH3-adjacent (slow Static engine perceived) | **Open.** Walkthrough revealed ~1 minute delay perceived as Static engine slowness. Profiled engine: ~25ms cold. Real cause: auto-simulate firing full Dynamic EP run on project-load saves. Fix-path needs Chris's call: (a) disable auto-simulate default; (b) gate on user-initiated saves only; (c) UI signalling improvement. See halt report Halt 3 for full options. Brief 28a Part 3 blocked until resolved. |
