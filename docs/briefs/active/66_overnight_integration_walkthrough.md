...
4. **Toggle consistency for the U4 bug class.** Disable ventilation. Does fan electricity go to zero? Disable lighting. Does lighting electricity go to zero? Disable each service one at a time, confirm all downstream numbers respond. List any that don't.
