...
| Status | **RESOLVED** — Brief 42 Part 4 (2026-05-20). The `scripts/42_systems_ux_migration.py --force` path re-seeds empty `lighting` and `small_power` arrays from the DEFAULT_PARAMS thin entries. Bridgewater's small_power is now populated post-migration (1 entry, baseline). The architectural fix (per-service load-side merge instead of whole-object `??`) is not landed — the migration-script-side re-seed is the pragmatic close that handles the field symptom without touching the load path. If the symptom recurs on a new project, run the migration with --force. The architectural improvement remains queued as a small follow-up if/when it becomes blocker. |
...
| Status | **RESOLVED via the lighter answer — Brief 43 widened the curated editor instead of wrapping arbitrary main-app UI.** (Closed 2026-05-20.) The curated editor in `InterventionEditorBuildingView` now exposes: service-level setpoints (heating + cooling + DHW temps + demand), per-system source / efficiency / share / control_mechanism / control_schedule_ref / enabled (all six services), ventilation per-system flow_rate + flow_rate_basis + sensible + latent recovery, lighting + small_power control_mechanism + control_factor, ground floor U construction picker, and structural ops (add / remove / replace systems per service). Brief 43 §3.7 verification matrix expanded to 16 rows (10 from Brief 41 + 6 new). The deferred "wrap arbitrary main-app UI" path remains hypothetically available as a much larger future brief if real use after Brief 43 surfaces gaps, but no concrete need has been identified. |
...
| Status | **RESOLVED** — Brief 42 (2026-05-20). DHW service-level fields (`dhw_storage_setpoint_c`, `dhw_tap_outlet_temp_c`, `dhw_cold_supply_temp_c`, `dhw_demand_basis`, `dhw_demand_litres_per_person_per_day`, `dhw_demand_litres_per_m2_per_day`) live directly on `systems_config_v40`; per-system DHW entries retain only `id / label / source / efficiency_metric / share_pct / control_mechanism / control_schedule_id / capacity_kw / notes / enabled`. Engine reads service-level (`_computeDhw` in `systemsEngine.js`); per-system shares partition delivered/fuel. UI exposes the demand block once per service in `ServiceSectionHeader.jsx`. Bridgewater migration: `scripts/42_systems_ux_migration.py` lifts lead-enabled-wins; disagreement is warned. Bridgewater's pre-existing gas/ASHP per-system disagreement (gas tap=30 vs ASHP tap=40, gas demand=80 vs ASHP demand=105) collapses to gas (lead-enabled) values: tap=30 / demand=80 — matches the pre-Brief-42 engine's `systems[0]`-lead behaviour. |
...
| Status | **RESOLVED** — Brief 42 Part 3 (2026-05-20). `SystemEditorPopout.jsx` (Brief 37 `SchedulePopout` chrome, draggable, position persisted at `nza-system-editor-popout-position`) hosts the refactored `SystemEditorCard` body. Left panel rows are now compact `SystemSummaryRow` instances: ●label · share% · headline efficiency · pencil-edit. Clicking the pencil opens the pop-out at full width. Building-level fields (setpoint, DHW temps/demand) lifted to `ServiceSectionHeader` so they aren't competing for system-card real estate. |
...
## #23 — Profiles annual electricity badge over-counts when heating delivered < demand

| Field | Value |
|---|---|
| Module | Systems (Profiles tab) — also affects any consumer of `daily_profiles.fuel_kwh_per_day.electricity` |
| Engine | Static `instantCalc.js` daily-profile aggregation |
| Severity | **S2** — number off in a defensible direction (it's "fuel that *would* be drawn if all heating demand were met by the SCOP-blended electric stack"), but two visible totals on the same page disagree without a UI cue. On Bridgewater the gap is 22.8 MWh (~8% of total electricity). On a project where unmet heating demand is larger, the gap grows linearly. |
| Status | **FIXED** by Brief 44 Part 5c (2026-05-21, this commit). Authorised as Option A in §13.8. Implementation: rescale demand-shaped daily arrays (`heating_demand_daily`, `cooling_demand_daily`) to brief40-delivered annual totals via `_scaleArr` helper before the `elec_daily` / `gas_daily` formulas consume them; also add `circulation_pump_kwh / 365` to the DHW electricity term to close a separate 1.05 MWh residual (recirc pump was counted in `consumption.total.electricity_mwh` but missing from the daily integral). Bridgewater post-fix four-way agreement: engine 283.053 = Profiles 283.053 to the kWh. Three deliberate-edit tests (T1 VRF off / T2 share 95→50 / T3 DHW tap 40→50) all PASS with four-way agreement (audit doc §13.8a). |
| Current value (Bridgewater, follow_comfort 21°C) | Profiles tab "Σ elec" badge: **305.890 MWh**. Live Results right-panel "Electricity": **283.053 MWh**. Gap: **22.837 MWh**. Engine canonical: `consumption.total.electricity_mwh = 283.053` (matched by Sankey carrier-column total 283.1, and matches `consumption.brief40.heating.delivered_total_mwh + …` rolled to fuel via per-system SCOP). |
| Root cause | `daily_profiles.fuel_kwh_per_day.electricity` is computed from heating **demand** (`heating_daily_delivered` array, sum = 90.099 MWh) divided by the blended `heat_scop_eff` (= 2.569 on Bridgewater), not from heating **actually delivered** (28.767 MWh). The variable name `heating_daily_delivered` in `instantCalc.js` is misleading — at this point in the dispatch it carries the demand-at-comfort integrand, not the v40 systems-delivered output. Arithmetic check: (90.099 − 28.767) / 2.569 = 23.873 MWh ≈ 22.837 MWh gap (minor residual = blended-SCOP rounding + cooling demand/delivered residual 0.047 MWh). |
| Why the protocol caught this | Three-way agreement test (Live Results ↔ Sankey ↔ Profiles) at baseline. Live Results and Sankey both read `consumption.total.*` (= engine canonical). Profiles "Σ elec" badge reads from a different code path (`daily_profiles.fuel_kwh_per_day.electricity`) and therefore diverged the moment heating was capacity-limited. Engine canonical wins per audit §13.7. |
| Why missed by §13.6 line-by-line presentation-only verification | §13.6 verified `InteractiveProfileVisualiser` is presentation-only (only kWh→kW unit conversion, no physics). That's still true — the **producer** of the daily profile is at fault, not the visualiser. The visualiser correctly displays what it's given. The bug is upstream in `instantCalc.js` daily aggregation. |
| Why missed by §13.2 gas-trace investigation | §13.2 traced `fuel_kwh_per_day.gas` and showed it's mathematically flat at 665 kWh/day for Bridgewater because `heat_gas_share = 0`. That trace did NOT exercise the electricity path. The electricity path uses `heat_elec_share = 1.0` for Bridgewater, multiplied by demand-side daily heating, producing the over-count. Same code shape (`heating_daily_delivered / heat_scop_eff × heat_elec_share`), opposite share value, divergent display number. |
| Location | `frontend/src/utils/instantCalc.js` line ~4485 — the daily-fuel formula. See audit doc §13.2 for the gas-side trace; the electricity-side is the symmetric formula immediately above/below it. |
| Suggested fix paths | **(a) Scale the daily fuel to match annual systems-delivered:** multiply `heating_daily_delivered_arr` by `(systems_delivered_total / demand_total)` before dividing by SCOP, so the daily-sum reconciles to v40-engine delivered. Lossy at the daily granularity but reconciles annual totals. **(b) Reshape the daily profile to use v40 per-system delivered:** compute per-system daily delivered from `consumption.brief40.heating.systems[*]` (which carries `delivered_mwh`), divide by per-system efficiency, sum into the daily electricity array. Correct at both daily and annual scales. Bigger change. **(c) Surface the demand-vs-delivered gap explicitly on the Profiles badge:** rename "Σ elec" to "Σ elec at demand" and add a second badge "Σ elec actual" reading `consumption.total.electricity_mwh`. UI-only, doesn't fix the daily-profile but makes the two numbers' difference visible. |
| Recommended | **(b)** — physically correct at both timescales and reuses the v40 per-system delivery the engine already produces. Audit doc §13.2.2 already flagged a related defensive-coding gap in `heat_gas_share` derivation; (b) would close both. |
| Cross-references | Audit doc §13.7 (protocol that surfaced this), §13.8 (test results, this issue), §13.2 (gas-trace investigation that nearly caught it), `instantCalc.js:~4485` (the formula), `consumption.brief40.heating.systems[*]` (engine v40 per-system delivered values), Brief 44 Part 6 close gated until #23 is resolved or explicitly accepted as Brief 44 → Brief 45 hand-off. |

---

## #24 — Boundary-mismatch follow-up bundle (deferred polish for a future small brief)

| Field | Value |
|---|---|
| Module | Static engine + Live Results panel + inline-legacy 'full' code path |
| Engine | `frontend/src/utils/instantCalc.js` |
| Severity | **S1** — all three items below are in the same family as Issue #23 (demand-vs-delivered boundary semantics) but lower-impact: defensive coding, code-clarity / architectural debt, and a UI display inconsistency. None blocks Brief 44 close. |
| Status | **OPEN — DEFERRED.** Authorised by Chris 2026-05-21 alongside the Part 5c Issue #23 fix: "Defer to a future small brief". Bundled here so a single future brief can address the boundary-mismatch family in one pass. |
| Item (a) — `heat_gas_share` defensive guard | `instantCalc.js:4474` derives `heat_elec_share = (elec_primary + elec_secondary) / heating.total_perf.fuel_mwh` with a `fuel_mwh > 0 ? … : 0` ternary. When `fuel_mwh > 0` but `delivered_mwh = 0` the share computes sensibly; but if `fuel_mwh = 0` (mid-edit transient — service disabled, all systems off, share-validation zero, etc.) the fallback `heat_elec_share = 0` causes `heat_gas_share = 1`. The daily formula then routes all heating demand into the gas channel — a momentary visual glitch even though the heating result is zero so the multiplication is also zero. Defensive fix: when `fuel_mwh = 0`, set BOTH shares to 0 instead of `(0, 1)`. Audit doc §13.2.2 flagged this when verifying §13.2 gas-trace. |
| Item (b) — inline-legacy 'full' consolidation | `frontend/src/utils/instantCalc.js` carries a third parallel envelope-physics implementation alongside State 1 and State 2 (Brief 39 audit `docs/audit/39_calculation_flow_map.md`, Option (a)). Rule 14 requires keeping all three in sync on every envelope-physics change. The follow-up brief Option (a) — extract systems block, collapse the inline-legacy path — was identified by Brief 39 as known architectural debt to be rationalised when timing allows. Lowest priority of the three; touches engine architecture but no user-visible behaviour change. |
| Item (c) — LiveResultsPanel heating denominator inconsistency | Live Results right rail shows "Heating 28.8 / 90.1 MWh" — delivered ÷ comfort-band demand. The Systems Diagnostic tab shows "Heating 28.8 / 28.8" — delivered ÷ service-setpoint demand (the v40 comfort-vs-setpoint diagnostic). On `follow_comfort` mode the two denominators differ (90.1 vs 28.8) because they answer different questions: "what would a system need to deliver to keep the comfort band" vs "what would a system need to deliver at this service's setpoint". Both are correct in their own frame, but seeing them side-by-side is confusing. Resolution options: (i) align labels (Live Results says "Heating 28.8 / 90.1 to comfort", Diagnostic stays "Heating 28.8 / 28.8 at setpoint"); (ii) Live Results uses the same setpoint-denominator as Diagnostic (loses the "unmet demand" cue); (iii) Live Results adds both numbers ("delivered 28.8 / 28.8 at setpoint · comfort gap +61.3 MWh"). UI decision, not a number bug. Same boundary-mismatch family as Issue #23 (both surface the difference between demand-at-comfort and delivered) — but presentationally rather than numerically. |
| Location | (a) `instantCalc.js:4474-4478` · (b) `instantCalc.js:~5087+` per `docs/audit/39_calculation_flow_map.md` · (c) `frontend/src/components/modules/systems/LiveResultsPanel.jsx` (the heating row's "/ 90.1" denominator) and `frontend/src/components/modules/systems/SystemsDiagnosticPanel.jsx` (the corresponding "/ 28.8" denominator on Diagnostic). |
| Why bundled | All three are downstream of the same conceptual error: blurring the line between "demand the building has" and "energy the systems deliver". Issue #23 fixed the daily-aggregation symptom; this bundle cleans up the three remaining presentation/architecture/defensive surfaces. None is independently urgent; bundled lets a single future brief reason about boundary semantics across the codebase. |
| Cross-references | Audit doc §13.2.2 (heat_gas_share gap), §13.8 (Issue #23 fix that exposed (a) and (c)), `docs/audit/39_calculation_flow_map.md` (inline-legacy consolidation Option (a)), `docs/audit/40_systems_library_schema.md` (Brief 40's comfort-vs-setpoint diagnostic that surfaced (c)). |
