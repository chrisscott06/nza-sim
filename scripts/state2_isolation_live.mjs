/**
 * scripts/state2_isolation_live.mjs
 *
 * Brief 27 Part 8 — state isolation regression test (live engine, State 2).
 *
 * Per the state contract:
 *   "State isolation is non-negotiable: a State 2 computation must produce
 *    identical output regardless of any value in operation/systems."
 *
 * State 2 admits occupancy.* + gains.* (v2.3 inputs) so those are NOT
 * forbidden here. The forbidden set is the State 3 territory (real systems)
 * and State 2.5 territory (operable windows). Legacy occupancy fields are
 * forbidden where superseded by the v2.3 occupancy/gains blocks.
 *
 * Bar: byte-identical via canonical JSON. Float tolerance is zero.
 *
 * Pair this with scripts/state2_isolation_epjson.py for the EP path.
 *
 * Usage:
 *   node scripts/state2_isolation_live.mjs [project_id]
...
import { FORBIDDEN_ENVELOPE_GAINS_INPUTS } from '../frontend/src/utils/stateMode.js'
...
// ── Absurd values for forbidden paths ────────────────────────────────────────
// Each chosen to be physically wrong if leaked into the State 2 calc. If the
// contract holds, setting these has zero effect on State 2 output.
const ABSURD = {
  // Legacy occupancy fields superseded by v2.3 — should not affect State 2.
...
function canonical(o) {
  ...
  // Skip Float32Array (hourly temperature trace) — large + structurally
  // equivalent to a Float64Array; JSON.stringify handles it predictably.
  if (ArrayBuffer.isView(o)) return JSON.stringify(Array.from(o))
...
for (const fpath of FORBIDDEN_ENVELOPE_GAINS_INPUTS) {
  if (!(fpath in ABSURD)) {
...
const combinedOut = runState2(b, s)
...
if (failures.length === 0) {
  console.log(`  ✓ ALL PASS — ${passed} scenarios, every State 2 output byte-identical`)
  console.log('═════════════════════════════════════════════════════════════════════')
  process.exit(0)
} else {
